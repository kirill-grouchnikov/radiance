...
- Use `BreadcrumbPathEvent.getSource()` to obtain the model (or just use the same `bar.getModel()` above) and then `BreadcrumbBarModel.getItems()` to get the currently selected path.
