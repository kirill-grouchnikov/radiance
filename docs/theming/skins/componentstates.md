## Radiance look and feel - component states
...
* The UI delegate that queries the component model will use accurate component states that account for all the relevant on and off facets - including the core facets defined in the `ComponentStateFacet` class. When this (perhaps elaborate) state is passed to `ContainerColorTokensBundle.getContainerTokens(ContainerColorTokensAssociationKind, ComponentState, ContainerType)` API, the procedure described above will match the this state to one of the "base" states defined in your skin, and use the matching color tokens.
